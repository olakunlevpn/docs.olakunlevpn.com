...
Docs.prototype.docsearch = function () {
  ...
  if (doc && api && api.key != '') {
    var search = docsearch({
      ...
      algoliaOptions: {
        'hitsPerPage': 5,
        'facetFilters': ['project:' + doc.id, 'version:' + doc.version]
      }
    });
...
Docs.prototype.handleClick = function (e, url) {
  url = url.replace('#pjax-container', ''); // url = url.replace('.com', '.app');

  var link = $('<a>', {
    href: url
  })[0]; // Reload if not the same protocol and host.

  if (location.protocol !== link.protocol || location.hostname !== link.hostname || link.href.indexOf('#') > -1 && stripHash(link) == stripHash(location)) {
    location.href = link.href;
  }

  $.pjax.click($.Event('click', {
    currentTarget: link
  }), '#pjax-container');
};
...
Docs.prototype.elements = function () {
  // Tables.
  this.content.find('table').addClass('table table-striped table-bordered table-hover table-condensed'); // Images.

  this.content.find('img').addClass('img-responsive img-thumbnail'); // Iframes.
...
Docs.prototype.navbar = function () {
  ...
  nav.find('>li').each(function () {
    ...
    if (header.length) {
      ...
      header.replaceWith('<a href="#" class="dropdown-toggle" data-toggle="dropdown">' + header.html() + ' <span class="caret"></span></a>');
    } else {
...
__webpack_require__(/*! C:\xampp\htdocs\Documenter\resources\js\app.js */"./resources/js/app.js");
module.exports = __webpack_require__(/*! C:\xampp\htdocs\Documenter\resources\sass\app.scss */"./resources/sass/app.scss");
